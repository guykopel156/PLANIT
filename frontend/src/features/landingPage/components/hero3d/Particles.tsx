...
const MIN_PARTICLE_SIZE = 0.015;
const PARTICLE_SIZE_RANGE = 0.025;
const MIN_PARTICLE_OPACITY = 0.3;
...
function generateParticleSizes(): number[] {
  return Array.from({ length: PARTICLE_COUNT }, () => MIN_PARTICLE_SIZE + Math.random() * PARTICLE_SIZE_RANGE);
}

function generateParticleOpacities(): number[] {
  return Array.from({ length: PARTICLE_COUNT }, () => MIN_PARTICLE_OPACITY + Math.random() * PARTICLE_OPACITY_RANGE);
}
