...
function HeroSection({ heroRef, heroProgress, mouse }: IHeroSectionProps): React.ReactElement {
  ...
  return (
    <section ref={heroRef} className="relative flex min-h-screen items-center justify-center overflow-hidden">
      <div className="absolute inset-0" style={{ transform: `scale(${scale})`, opacity: fade, willChange: 'transform, opacity' }}>
        <Suspense fallback={null}>
          <FloatingShapes />
        </Suspense>
      </div>

      <div className="absolute inset-0 bg-gradient-to-b from-white/70 via-white/40 to-white dark:from-[#030712]/60 dark:via-[#030712]/30 dark:to-[#030712]" />

      <div className="pointer-events-none absolute -left-40 top-1/3 h-[600px] w-[600px] rounded-full bg-cyan-500/10 blur-[140px] dark:bg-cyan-500/20" />
      <div className="pointer-events-none absolute -right-40 bottom-1/4 h-[500px] w-[500px] rounded-full bg-blue-500/10 blur-[120px] dark:bg-blue-500/20" />
      ...
      <div className="absolute bottom-8 left-1/2 z-10 -translate-x-1/2" style={{ opacity: Math.max(0, 1 - heroProgress * SCROLL_INDICATOR_FADE_SPEED) }}>
        <div className="flex h-10 w-6 items-start justify-center rounded-full border border-gray-300/40 p-1.5 dark:border-white/15">
          <div className="h-2 w-1 animate-bounce rounded-full bg-gray-400 dark:bg-white/50" />
        </div>
      </div>
    </section>
  );
